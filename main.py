...
from cv2 import face  # Add this line to ensure face module is loaded correctly
...
def TrainImages():
    check_haarcascadefile()
    assure_path_exists("TrainingImageLabel/")
    recognizer = cv2.face.LBPHFaceRecognizer_create()  # Corrected
    harcascadePath = "haarcascade_frontalface_default.xml"
    detector = cv2.CascadeClassifier(harcascadePath)
...
def TrackImages():
    ...
    # Make sure cv2.face is available after installing opencv-contrib-python
    recognizer = cv2.face.LBPHFaceRecognizer_create()  # Corrected
